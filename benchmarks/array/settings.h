...
void Settings::parseArguments(int argc, char** argv) {
   OptionMap parsers = {
       ...
       {'c',
        OptionPtr(new Option<uint64_t>(controllerId, "fdid", "ctrl",
                                       "NVM controller device identifier"))},
       {'f', OptionPtr(new Option<uint64_t>(cudaDeviceId, "fdid", "fdid",
                                            "CUDA device FDID"))},
       {'a', OptionPtr(new Option<uint32_t>(adapter, "number", "adapter",
                                            "DIS adapter number", "0"))},
       {'S',
        OptionPtr(new Option<uint32_t>(segmentId, "offset", "segment",
                                       "DIS segment identifier offset", "0"))},
#else
   ...
   // controller device path"))},
       ...
       // statistics", "false"))},
       {'n', OptionPtr(new Range(numReqs, 1,
                                 (uint64_t)std::numeric_limits<uint64_t>::max,
                                 "reqs", "number of reqs per thread", "1"))},
       {'p', OptionPtr(new Range(numPages, 1,
                                 (uint64_t)std::numeric_limits<uint64_t>::max,
                                 "pages", "number of pages in cache", "1024"))},
       ...
       {'e', OptionPtr(new Range(
                 numElems, 1, (uint64_t)std::numeric_limits<uint64_t>::max,
                 "num_elems", "number of 64-bit elements in backing array",
                 "2147483648"))},
       {'r', OptionPtr(
                 new Option<bool>(random, "bool", "random",
                                  "if true the random access benchmark runs, if "
                                  "false the sequential access benchmark runs",
                                  "true"))},
       ...
       {'S',
        OptionPtr(new Range(ssdtype, 0, 2, "ssd",
                            "type of SSD to use 0->Samsung, 1->Intel", "0"))},
