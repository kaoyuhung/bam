...
static void GPUMEM_TEST(const Settings &settings, size_t gpu_free_mem) {
  ...
  for (uint64_t i = 0; i < n_elems_size + 16; i += buf_size) {
    ...
    read(fda, a_h, bytes2read);
    read(fdb, b_h, bytes2read);
    if (i == 0) {
      itr_n_elems_size -= 16;
...
static void UVM_DIRECT_TEST(const Settings &settings, uint64_t sys_free_mem) {
  ...
  for (uint64_t i = 0; i < (n_elems_size + buf_size - 1) / buf_size; i++) {
    uint64_t itr_n_elems_size = min(buf_size, n_elems_size - i * buf_size);
    uint64_t bytes2read = round_to_nearest(itr_n_elems_size, 4096);
    pread(fda, a, bytes2read, 16 + i * buf_size);
    pread(fdb, b, bytes2read, 16 + i * buf_size);
    if (type == BASELINE) {
      kernel_baseline<<<blockDim, numthreads>>>(
          itr_n_elems_size / sizeof(uint64_t), a, b, sum);
    } else if (type == OPTIMIZED) {
    ...
    cuda_err_chk(cudaMemcpy(a, sum, itr_n_elems_size, cudaMemcpyDeviceToHost));
    pwrite(fdc, a, bytes2read, i * buf_size);
  }
  cudaDeviceSynchronize();
